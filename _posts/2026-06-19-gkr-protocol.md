...
- **Soundness**: If $$x \notin L$$, no (possibly malicious) prover $$P^\star$$ convinces $$V$$ with probability greater than a soundness error $$\varepsilon_s$$.
...
| Check | Security requirement | Failure mode if violated |
|:---:|------------|------------|
| ☐ | Every challenge $$r_j$$ and $$t^\star$$ is sampled uniformly from $$\mathbb{F}$$ by a CSPRNG, after the corresponding prover message has been received. | A predictable or early-released challenge lets the prover back-fit a polynomial that passes exactly at the checkpoint and lies elsewhere. |
...
| ☐ | The line-restriction step verifies $$q(0) = a$$ and $$q(1) = a'$$ before the random $$t^\star$$ is drawn, and rejects if $$\deg q \gt m$$. | An unchecked line polynomial lets the prover substitute an unrelated claim and discard the two real ones. |

...
image: /assets/article/blockchain/ethereum/eip-7702/2025-05-09-eip-7702-overview-mindmap.png
isMath: false
---
...
The simplest pointer would just be the address of code deployed on-chain.
...
EIP-7702 introduces the type `0x04` set-code transaction, which lets an EOA install a delegation indicator (`0xef0100 || address`) in its own code so that all code-executing operations run the code at the delegated address in the EOA's context. The design favours persistent delegation by template (a pointer to deployed code, not inline bytecode) to keep EOA and smart-contract-wallet workstreams unified, and it is forward-compatible with ERC-4337. The trade-offs are concentrated in security and propagation: a delegate contract must itself sign over nonce, value, gas, and target to avoid handing control to a malicious caller, initialization can be front-run, storage layouts must avoid collisions across migrations, and the broken `tx.origin == msg.sender` invariant affects contracts that relied on it.

![EIP-7702 set code for EOAs mindmap]({{site.url_complet}}/assets/article/blockchain/ethereum/eip-7702/2025-05-09-eip-7702-overview-mindmap.png)

```
@startmindmap
* EIP-7702: Set Code for EOAs
** Motivation
*** Batching (atomic multi-op)
*** Sponsorship (X pays for Y)
*** Privilege de-escalation (sub-keys)
** Mechanism
*** New tx type 0x04 (SET_CODE_TX_TYPE)
*** authorization_list of tuples
**** [chain_id, address, nonce, y_parity, r, s]
*** Delegation indicator: 0xef0100 || address
*** chain_id = 0 -> all chains
** Behavior
*** Processed before execution, after nonce++
*** ecrecover authority; verify nonce and code
*** CODESIZE / CODECOPY use executing code
*** Precompile target -> empty code
*** Delegation chains: follow only first
** Gas
*** PER_AUTH_BASE_COST = 12500
*** PER_EMPTY_ACCOUNT_COST = 25000
*** Worst-case charged upfront, refund if account exists
** Backwards Compatibility
*** Balance can decrease via external call
*** Nonce can increase mid-tx
*** tx.origin == msg.sender invariant broken
** Security Considerations
*** Delegate must sign nonce / value / gas / target
*** Front-running initialization
*** Storage collisions on migration (ERC-7201)
*** Sponsored relayer griefing
*** Transaction propagation limits
@endmindmap
```

## Frequently Asked Questions

**Q: What does EIP-7702 actually change about an EOA?**

It lets an EOA set its own account code to a *delegation indicator*, the 23-byte value `0xef0100 || address`. Once set, every code-executing operation (CALL, DELEGATECALL, a transaction whose destination is the EOA, and so on) loads and runs the code at `address` in the context of the EOA. The EOA keeps its key and balance but can now behave like a smart-contract wallet.

**Q: What is in an authorization tuple, and what does `chain_id = 0` mean?**

Each tuple is `[chain_id, address, nonce, y_parity, r, s]`: the target code address, the authorizing account's expected nonce, and a secp256k1 signature over `keccak(MAGIC || rlp([chain_id, address, nonce]))`. Setting `chain_id` to a specific chain scopes the authorization to that chain only; setting it to `0` makes the authorization valid on any chain, which is convenient when the same code is deployed deterministically everywhere but removes cross-chain malleability protection.

**Q: Why does EIP-7702 use a pointer to code (creation by template) instead of inline bytecode?**

Cost and unification. A pointer is a 20-byte address, whereas the smallest useful proxy bytecode is around 50 bytes, so storing code inline would waste roughly 30 bytes replicated across many accounts for no functional gain. Specifying code directly would also give EOAs a new ability to run arbitrary calldata-supplied code, splitting UX development into two competing workstreams (EOA scripts versus smart-contract wallets), which the EIP deliberately avoids.

**Q: How does EIP-7702 break the `tx.origin == msg.sender` invariant, and who is affected?**

Self-sponsoring lets `tx.origin` set and run its own delegated code, so `msg.sender == tx.origin` can now be true below the topmost frame. This breaks checks that used the equality for three purposes: detecting that the caller is an EOA (unaffected, since `tx.origin` is still an EOA), atomic-sandwich/flash-loan protection (broken, though relying on `tx.origin` this way was already discouraged), and reentrancy guards of the form `require(tx.origin == msg.sender)` (broken, but the authors found no real examples of this pattern).

**Q: What is the single most important responsibility of a delegate contract?**

It must require and verify a signature from the account's authority over the security-critical parameters of any operation it performs, specifically a replay-protection nonce, plus `value`, `gas`, and `target`/`calldata`. Without these, a malicious actor or sponsor can replay a signature, redirect calls to arbitrary contracts, or grief the user. A poorly implemented delegate can hand near-complete control of the signer's EOA to an attacker.

## References

- [EIP-7702: Set Code for EOAs (official specification)](https://eips.ethereum.org/EIPS/eip-7702)
